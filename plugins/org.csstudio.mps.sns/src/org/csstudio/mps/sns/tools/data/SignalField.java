...
public class SignalField extends RDBData implements Cloneable 
{
   ...
   */
  public final static String VALUE_COLUMN_NAME = "FLD_VAL";
  /**
   * Holds the name of the RDB column for the type property.
   */
  public final static String TYPE_COLUMN_NAME = "FLD_ID";
  /**
   * Holds the name of the RDB column for the class.
   */
  public final static String SIGNAL_FIELD_TABLE_NAME = "SGNL_FLD";
  ...
  public SignalField(ResultSet data) throws java.sql.SQLException
  {
    ...
    if(Arrays.binarySearch(columnNames, TYPE_COLUMN_NAME) >= 0)
      setType(new SignalFieldType(data));
    resetChangedFlag();
  }
  ...
  public boolean equals(Object obj)
  {
    ...
    if(! MPSBrowserView.compare(getType(), compareTo.getType()))
      return false;
    //Compare values
    if(! MPSBrowserView.compare(getValue(), compareTo.getValue()))
      return false;
    //Check in database flag
    if(isInDatabase() ^ compareTo.isInDatabase())
      return false;
    //Check the signal.
    ...
    else
    {
      if(compareSignal == null)
        return false;
      if(! signal.getID().equals(compareSignal.getID()))
        return false;
    }
    return true;
  }
  ...
  public int hashCode()
  {
    ...
    if(signal == null)
      ...
      hashCode = hashCode * 37 + findPropertyHashCode(signal.getID());
    return hashCode;
  }
  ...
  protected Object clone()
  {
    ...
    if(type != null)
      clone.setType((SignalFieldType)type.clone());
  ...
  {
    if(rdbFieldName.equals(VALUE_COLUMN_NAME))
      return getValue();
  ...
  {
    if(rdbFieldName.equalsIgnoreCase(VALUE_COLUMN_NAME))
