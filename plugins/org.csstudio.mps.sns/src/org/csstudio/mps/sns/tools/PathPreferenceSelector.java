...
public class PathPreferenceSelector extends JDialog implements ScrollPaneConstants {
    ...
    protected void initComponents() {
        ...
        row.add( new JLabel("Path: ") );
        pathField = new JTextArea();
        pathField.setColumns(40);
		pathField.setLineWrap(true);
		JScrollPane scrollPane = new JScrollPane(pathField, VERTICAL_SCROLLBAR_AS_NEEDED, HORIZONTAL_SCROLLBAR_NEVER);
        row.add(scrollPane);
    ...
    protected void commitChanges() {
		try {
			chooser.setDefaultPath( pathField.getText() );     // make this file the new default
			updateView();
		}
    ...
    protected void revertPath() {
		try {
			pathField.setText( chooser.getDefaultPath() );     // make this file the new default
			updateView();
		}
	...
	public boolean hasUnsavedChanges() {
		try {
			return !pathField.getText().equals( chooser.getDefaultPath() );
		}
		catch(MalformedURLException exception) {
			return true;
		}
...
class CustomChooser extends JFileChooser {
    ...
    public CustomChooser(final Preferences defaults, final String urlKey, final String suffix, final String description) {
		...
		try {
			String defaultFilePath = getDefaultPath();
