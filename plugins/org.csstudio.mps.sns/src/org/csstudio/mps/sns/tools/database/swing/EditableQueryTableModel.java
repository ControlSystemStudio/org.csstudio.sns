...
public class EditableQueryTableModel extends AbstractTableModel implements DatabaseTableModel
{
  ...
  {
    if(rowIndex == editRow && editedColumns.contains(columnIndex))
      return editRowData[columnIndex];
  ...
  public void refresh(String message)
  {
    ...
    if(SwingUtilities.isEventDispatchThread())
      refreshInThread(message);
    else
      refreshNow(message);
  }

  /**
   * Launches a <CODE>Thread</CODE> to refresh the data.
   * 
   * @param message The message to use in the progress events that are fired by the method.
   */
  private void refreshInThread(final String message)
  {
   ...
   * @param message The message to use in the progress events that are fired by the method.
   */
  private void refreshNow(String message)
  {
    try
    {
      rdbLock.lock();
      ...
      try 
      {
        ...
        try 
        {
          ...
          //First make sure column names are correct.
          if(columnNamesChanged)
          {
            if(newSchemaNames == null)
            {
              //No table names, schemas, or column names were given. Try to
              //get the information from the RDB.
              ...
              {
                //Schema and table names given, but no column information.
                //Get the column names from the result set and match them
                //with the table and schema names given.
          ...
          while(data.next())
          {
            ...
            for(int i=0;i<currentRow.length;i++) 
              currentRow[i] = data.getObject(i + 1);
    ...
    for(int i=0;i<newTableNames.length;i++) 
    {
      ...
      if(cachedColumns == null)
      {
        ...
        while(columnsData.next())
          cachedColumns.add(columnsData.getString("COLUMN_NAME"));
        cachedTableColumns.put(cachedTableName, cachedColumns);
      }
      int cachedColumnCount = cachedColumns.size();
      for(int j=0;j<cachedColumnCount;j++) 
        mappedColumnTables.put(cachedColumns.get(j), new String[]{newSchemaNames[i], newTableNames[i]});
    }
  ...
  /**
   * Creates a new <CODE>Thread</CODE> and commits the changes.
   */
  ...
  /**
   * Commits the changes in the current <CODE>Thread</CODE>.
   */
  ...
  /**
   * Creates a new <CODE>Thread</CODE> and rolls back any pending changes.
   */
  private void rollbackLater()
  {
    SwingUtilities.invokeLater(new Runnable()
    {
      public void run()
  ...
  /**
   * Rolls back any changes in the current <CODE>Thread</CODE>. This method 
   * needs to be called from the event <CODE>Thread</CODE>.
   */
  private void rollbackNow()
  {
    if(isChanged())
      cancel();
    SwingWorker worker = new SwingWorker()
  ...
  {
    //Sanity check. Make sure value has changed.
    if(compare(aValue, getValueAt(rowIndex, columnIndex)))
  ...
  private void changesPosted()
  {
    if(isChanged())
    {
      //Move the data in the edit row into the main data.
  ...
  public void setQuery(String query, String[] schemaNames, String[] tableNames, String[] columnNames)
  {
    if(schemaNames != null)
    {
      if(tableNames == null || tableNames.length != schemaNames.length || (columnNames != null && columnNames.length != schemaNames.length))
        throw new java.lang.IllegalArgumentException("Arrays passed in need to be the same length.");
    }
    else
    {
      if(tableNames != null || columnNames != null)
        throw new java.lang.IllegalArgumentException("Arrays passed in need to be the same length.");
    }
    this.query = query;
    ...
    try
    {
      ...
      for(int i=0;i<editedColumnCount;i++) 
      {
        ...
        if(schemaName.equals(schemaNames[editedColumnIndex]) && tableName.equals(tableNames[editedColumnIndex]))
        {
          if(queryValues.size() > 0)
            buffer.append(", ");
          buffer.append(getColumnName(editedColumnIndex));
          buffer.append(" = ?");
      ...
      try 
      {
        ...
        for(int i=0;i<valueCount;i++) 
          query.setObject(i + 1, queryValues.get(i));
        query.execute();
  ...
  private int findColumnIndex(String schemaName, String tableName, String columnName)
  {
    ...
    {
      if(schemaName.equals(schemaNames[i]) && tableName.equals(tableNames[i]) && columnName.equals(columnNames[i]))
        return i;
    }
    return -1;
  }
  ...
  /**
   * Cancels the changes in the event thread using 
  ...
  /**
   * Cancels the changes in the current <CODE>Thread</CODE>.
   */
  ...
  public String[] getPrimaryKeys(String schemaName, String tableName) throws java.sql.SQLException
  {
    ...
    if(keys == null)
    {
      ...
      try 
      {
        ...
        while(result.next())
          rdbKeys.add(result.getString("COLUMN_NAME"));
        keys = (String[])rdbKeys.toArray(new String[rdbKeys.size()]);
        //Remember these so we don't have to go back later.
  ...
  /**
   * Fires a progress change event.  This is done in a thread safe manner using
   * <CODE>SwingUtilities.invokeLater</CODE>.
   * 
   * @param e The <CODE>ProgressEvent</CODE> to fire.
   */
  ...
  /**
   * Fires a task started event.
   * 
   * @param e The <CODE>ProgressEvent</CODE> to fire.
   */
  private void fireTaskStartedEvent(ProgressEvent e)
  {
    List listeners = (List)progressListeners.clone();
    int count = listeners.size();
   ...
   * @param e The <CODE>ProgressEvent</CODE> to fire.
   */
  protected void fireTaskInterruptedEvent(final ProgressEvent e)
  {
    List listeners = (List)progressListeners.clone();
    int count = listeners.size();
   ...
   * @param e The <CODE>ProgressEvent</CODE> to fire.
   */
  protected void fireTaskCompleteEvent(final ProgressEvent e)
  {
    List listeners = (List)progressListeners.clone();
    int count = listeners.size();
  ...
  /**
   * Fires the given property change event.
   * 
  ...
  /**
   * Handles the firing of the <CODE>PropertyChangeEvent</CODE>.
   * 
  ...
  public void setColumnReadOnly(String schemaName, String tableName, String columnName, boolean readOnly)
  {
    ...
    if(readOnly)
    {
      if(readOnlyColumns.contains(columnReference))
        readOnlyColumns.add(columnReference);
    }
    else
      readOnlyColumns.remove(columnReference);
  }
}
