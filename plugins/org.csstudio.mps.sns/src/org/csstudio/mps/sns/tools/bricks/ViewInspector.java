...
public class ViewInspector extends JDialog {
	/** single inspector instance */
	protected final static ViewInspector INSPECTOR;
	...
	// static initializer
	static {
		INSPECTOR = new ViewInspector();
	}
	
	
	/** Constructor */
	protected ViewInspector() {
		setTitle( "View Inspector" );
		setSize( 400, 600 );
	 ...
	 */
	public static ViewInspector getInspector() {
		return INSPECTOR;
	}
	 ...
	 */
	public static void inspect( final BeanNode node ) {
		INSPECTOR.setViewNode( node );
		
		if ( !INSPECTOR.isVisible() ) {		// need to check this to prevent the inspector from coming to the front if it is already visibile
			INSPECTOR.setVisible( true );
		}
	}
	
	
	/** display the default view palette */
	static public void inspectBeanNear( final BeanNode node, final Component reference ) {
		if ( !INSPECTOR._hasBeenPositioned ) {
			INSPECTOR.setLocationRelativeTo( reference );
			INSPECTOR._hasBeenPositioned = true;
		}
		inspect( node );
	}
	 ...
	 * Set the view node
	 */
	public void setViewNode( final BeanNode node ) {
		_node = node;
		...
		final PropertyValueCellEditor editor = new PropertyValueCellEditor( PARAMETER_TABLE, PROPERTY_EDITOR_MANAGER, tableModel );
	...
	protected void makeContent() {
		...
		mainView.add( makeSettingsView() );
		mainView.add( makeBeanPropertiesView() );
	}
