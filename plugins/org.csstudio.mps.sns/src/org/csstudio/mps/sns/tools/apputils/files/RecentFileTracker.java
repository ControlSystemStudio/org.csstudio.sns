...
package org.csstudio.mps.sns.tools.apputils.files;

import org.csstudio.mps.sns.tools.StringJoiner;
...
public class RecentFileTracker {
	...
	public File getRecentFolder() {
		final String recentFilePath = getMostRecentFilePath();		
		return recentFilePath != null ? new File(recentFilePath).getParentFile() : null;
	}
	...
	public File getMostRecentFile() {
		final String recentFilePath = getMostRecentFilePath();		
		return recentFilePath != null ? new File(recentFilePath) : null;
	}
	
	
	/**
	 * Get the most recent file path
	 * @return the file path of the most recently cached URL.
	 */
	private String getMostRecentFilePath() {
		final String[] recentURLSpecs = getRecentURLSpecs();
		String recentSpec = recentURLSpecs.length > 0 ? recentURLSpecs[0] : null;
		try {
			return recentSpec != null ? new URL(recentSpec).getPath() : null;
		}
		catch(MalformedURLException exception) {
			return null;
		}
