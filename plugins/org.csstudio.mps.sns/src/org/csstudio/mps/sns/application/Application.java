	...
	final protected void registerApplicationStatusService() {
		...
		if ( shouldRegister.booleanValue() ) {
			try {
				...
				Logger.getLogger( "org.csstudio.mps.sns.application" ).log( Level.INFO, "Registered application services..." );
			}
			catch(Exception exception) {
				exception.printStackTrace();
				System.err.println("Service registration failed due to " + exception);
				Logger.getLogger( "org.csstudio.mps.sns.application" ).log( Level.SEVERE, "Service registration failed...", exception );
			}
		}
		else {
